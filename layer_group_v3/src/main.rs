...
lazy_static! {
    ...
    static ref NEW_DROPPABLES: Mutex<HashMap<String, Mutex<TestDroppable>>> = Mutex::new(HashMap::new()) ;
}
...
fn main() {
    ...
    NEW_DROPPABLES.lock().unwrap().insert(String::from("layer container 1"), Mutex::new(TestDroppable::from(container_1)));

    //droppable buttons
    let mut drop_trash = DroppableButton::new(50,50,"trash", "assets/delete_black_24dp.svg");
    NEW_DROPPABLES.lock().unwrap().insert(String::from("trash"), Mutex::new(TestDroppable::from(drop_trash)));
    let mut copy_button = DroppableButton::new(50,50,"trash", "assets/content_copy_black_24dp.svg");
    NEW_DROPPABLES.lock().unwrap().insert(String::from("trash"), Mutex::new(TestDroppable::from(copy_button)));
    ...
    NEW_DROPPABLES.lock().unwrap().insert(String::from("layer container 2"), Mutex::new(TestDroppable::from(container_2)));

...
lazy_static! {
    ...
    // static ref NEW_DROPPABLES: Mutex<HashMap<String, D>> = Mutex::new(HashMap::new());
    static ref NEW_DROPPABLES: Mutex<Drops<TestDroppable>> = Mutex::new(Drops::new());
    // static 
}


struct Drops<D: DroppableExt> {
    map: HashMap<String, D>
}

impl<D: DroppableExt> Drops<D>{
    pub fn new()->Self{
        Drops{
            map: HashMap::new(),
        }
...
fn main() {
    ...
    let mut my_layer_1 = LayerDraggable::new(100,100,100,50, "layer 1", None);
    container_1.add_child(Box::new(my_layer_1.clone()) as DragObject);
    dbg!(my_layer_1.parent_id());
    let mut my_layer_2 = LayerDraggable::new(100,100,100,50, "layer 2", None);
    container_1.add_child(Box::new(my_layer_2) as DragObject);
    let mut my_layer_3 = LayerDraggable::new(100,100,100,50, "layer 3", None);
    ...
    let mut copy_button = Box::new(DroppableButton::new(50,50,"trash", "assets/content_copy_black_24dp.svg")) as DropObject;
    DROPPABLES.lock().unwrap().insert(String::from("trash"), copy_button);
    ...
    //     }
    // });
    // let testcont = LayerContainer::new(100,100,200,200,"TEST");


    let mut container_2= Box::new(LayerContainer::new(700,300,300,600, "layer container 2")) as DropObject;
    let mut my_layer_4 = LayerDraggable::new(100,100,100,50, "layer 4", None);
    container_2.add_child(Box::new(my_layer_4) as DragObject);
    let mut my_layer_5 = LayerDraggable::new(100,100,100,50, "layer 5", None);
    container_2.add_child(Box::new(my_layer_5) as DragObject);
    let mut my_layer_6 = LayerDraggable::new(100,100,100,50, "layer 6", None);
                        ...
                        (Some(source), Some(target))=>{
                             match (source.draggable_type.clone(), target.draggable_type.clone()){
                                ...
                                (DraggableType::Layer, DraggableType::Layer)=>{
                                    ...
                                    match DROPPABLES.lock().unwrap().get_pair_mut(&source.parent, &target.parent){
                                ...
                                (DraggableType::Layer, DraggableType::Container)=>{
                                    ...
                                    match DROPPABLES.lock().unwrap().get_pair_mut(&source.parent, &target.draggable_id){
                                        Some((source_parent, target_parent))=>{
                                            let to_swap = source_parent.remove_child(source.draggable_id).unwrap();
                                            target_parent.add_child(to_swap.1);
                                        }

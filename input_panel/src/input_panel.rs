...
use crate::{CustomInputValue, Distance};
use super::{Message, InputWrapper};
...
impl <'a> InputPanel <'a>{
    pub fn new(x: i32, y: i32, w: i32, h: i32, label: String) -> Self {
        ...
        let mut header = Frame::new(x,y,w,20,&label);
        test_group.add(&header);
        // pack.add(&header);

        test_group.add(&pack);
        pack.resize(test_group.x()+15, test_group.y()+20, test_group.width()-30, test_group.height()-5);
        ...
        // sb.pack.handle2(move|t, ev|{
        //     match ev {
        //         Event::KeyDown=>{
        //             match app::event_key(){
        //                 Key::Tab=>{
        //                     dbg!("hit tab key in input panel");
    ...
    pub fn add_input(&mut self, input_wrapper: &'a InputWrapper){
        ...
        // input_wrapper.input.redraw_input();
        self.padding_pack.resize(self.padding_pack.x(), self.padding_pack.y(), self.padding_pack.width(), self.padding_pack.height()+&input_wrapper.height());
